// src/components/client/OrderDetail.jsx
...
export class OrderDetail extends Component {
  ...
  cargarDetallePedido = async () => {
    ...
    try {
      ...
      this.setState({ pedido: data, cargando: false });
    } catch (error) {
      console.error("❌ Error al cargar detalle del pedido:", error.message);
      this.setState({
        pedido: null,
        cargando: false,
        error: error.message,
  ...
  render() {
    const { pedido, cargando, error } = this.state;
          ...
          {cargando ? (
            ...
            <div className="bg-[#fef3c7] p-12 rounded-2xl border-4 border-[#854d0e] shadow-[6px_6px_0px_0px_rgba(0,0,0,0.2)] text-center">
              ...
              <div className="flex gap-3 justify-center">
                <button
                  onClick={this.cargarDetallePedido}
              ...
              <div className="p-6 border-b-4 border-[#854d0e]/20">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  ...
                  {(() => {
                    ...
                    return (
                      <div
                        className={`flex items-center gap-2 px-4 py-2 rounded-full border ${estado.bg} border-[#854d0e]/30`}
                      >
                        <EstadoIcono className={`w-5 h-5 ${estado.color}`} />
                        <span className={`font-bold ${estado.color}`}>
                          {estado.texto}
                        </span>
                      </div>
                    );
